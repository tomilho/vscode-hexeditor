...
const DataRowContents: React.FC<{
	...
	const { bytes, chars } = useMemo(() => {
		...
		for (let i = 0; i < width; i++) {
			...
			for (; j < decorators.length && !decorators[j].range.includes(boffset); j++);
			...
			bytes.push(
				<DataCell
					key={i}
					className={clsx(j < decorators.length && HexDecoratorStyles[decorators[j].type])}
					offset={boffset}
					isChar={false}
					isAppend={false}
					value={value}
				...
				chars.push(
					<DataCell
						...
						className={clsx(
							...
							j < decorators.length && HexDecoratorStyles[decorators[j].type],
						)}
						value={value}
					>
						{char === undefined ? "." : char}
